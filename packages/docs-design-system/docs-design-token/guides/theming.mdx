...
When you apply `.dg-theme-dark` to an element, all its descendants automatically use dark theme values — **no code changes needed** in your component styles.
...
| `getOppositeTheme(theme)` | `ThemeName` | `ThemeName` | Returns `'light'` ↔ `'dark'` |
