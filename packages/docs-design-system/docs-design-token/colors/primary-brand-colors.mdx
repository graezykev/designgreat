...
Primary colors define link appearance and interactive states.
...
Primary colors are used for checkboxes, toggles, and input focus states.
...
The `color.primary.focus.shadow` token powers the focus ring effect.

→ **See live demos**: [Focus Indicators](./shortcuts/shadow#focus-indicators)
