...
/* Tertiary action - moderate importance */
.button-tertiary {
  background-color: var(--dg-color-tertiary-default);
}

/* Quartus action - least important */
.button-quartus {
  background-color: var(--dg-color-quartus-default);
}
```
...
### Badges with Different Priorities
...
.badge-quartus {
  background-color: var(--dg-color-quartus-subtler);
  color: var(--dg-color-quartus-default);
}
```
...
- [Primary Colors](./primary-brand-colors) - Primary action colors
- [Accent Colors](./accent-colors) - Understanding accent color gradients
- [Semantic Colors](./semantic-colors) - Colors with specific meanings
- [Theme Switching](/design-token/guides/theme-switching) - Theming these color sets
