...
- All Storybook/dev commands regenerate tokens automatically
...
Storybook starts on `http://localhost:6006`. Use the **Theme** toolbar toggle (sun/moon icon) to switch between light and dark palettes. Stories render inside `dg-theme-*` wrappers, so Tailwind token utilities update instantly when the theme changes.
...
- **Theme desync:** If toggling light/dark does nothing, ensure the preview iframe body contains the expected `dg-theme-*` class. Re-running the dev server typically fixes it
- **Token mismatch:** Re-run `generate:theme` or blow away `node_modules/.cache` if Tailwind refuses to emit a new token class
