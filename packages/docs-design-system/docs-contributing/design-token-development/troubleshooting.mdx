...
**Solution:** Remove circular dependencies. Use base tokens that don't reference each other:

```javascript
// ✅ Good: Base token with no reference
color.base.blue = { value: '#3B82F6' }

// ✅ Good: One-way reference to base
color.primary = { value: '{color.base.blue}' }
```
...
cat dist/css/light/variables.css | grep 'primary'

# Test in isolation
pnpm run build:tokens   # Token generation only
pnpm run typecheck      # Type check only
pnpm run validate       # Validation only
```

### Performance

- Use `pnpm run watch` for development (faster iteration)
- Use `pnpm run build:tokens` for quick builds (skips TypeScript)
- Check for duplicate tokens if bundle size is large
